...
Indie::GameComponents::APlayer::APlayer(
    Map &map, Vector2 position, std::array<KeyboardKey, 5> keyMap, std::string texturePath, std::string modelPath, Color color, std::string modelBombPath, std::string modelBombAnimationPath, std::string modelExplosionPath
): _Map(map), _keyMap(keyMap), _texture(texturePath), _model(modelPath, _texture), _color(color), _modelBomb(modelBombPath, _texture), _modelBombAnimationPath(modelBombAnimationPath), _modelExplosion(modelExplosionPath, _texture)
{
